...
### **BUG FIX:** Updating `AssemblyInstruction::generateCode()`

One of the properties of `Config` is `uint16_t start_address`, i.e., the start
address of the assembled program (the address at which it should be loaded into
memory).

Since this property is set by the Preprocessor, only it and pipeline stages
after it (i.e., the Label Handler and File Generation) can read the property's
value (if it differs from the default `0x0000` start address).

I originally thought that the only entity that needed to read the start address
was the Label Handler, which would add the `start_address` as an offset to the
instruction addresses generated by the Parser (which assumes, effectively, that
`start_address` is `0x0000`). The Label Handler would convert all labels to
immediates that are shifted forward by the `start_address`, which should
therefore cause the File Generation stage to generate code with corrected
instruction addresses.

This was mostly true, but a slight oversight was made - the assumption I made
here was that instruction addresses would only be referenced whenever a label
was used in the assembly code - i.e., in branch or jump instructions. The
oversight was in regards to the `CALL` instruction, which, in addition to
jumping to an address specified by a label (which was therefore offset
correctly by the Label Handler), pushed the *return address* onto the stack.

However, no label is explicitly used to specify the return address - instead, it
is computed with reference to the `CALL` instruction's address. Since this
address doesn't come from an Immediate token that used to be a Label, it wasn't
an instruction address generated by the Label Handler; instead, it was the
instruction address of the `CALL` instruction *computed by the Parser*, and
so was not offset by the start address specified in the `Config`. (Of course,
the return address is the address of the instruction after the `CALL`
instruction, or rather the address of the instruction immediately after the last
ISA instruction that the `CALL` assembly instruction is converted into).

To fix this, I changed the function signature of
`AssemblyInstruction::generateCode()` to take a `Config &` argument and offset
the computed return address with the `start_address` specified by the `Config`
object. The reference to the `Config` object owned by the `Assembler` is passed
to `generateCode()` by the File Generation pipeline stage code.