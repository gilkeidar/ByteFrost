...
void Preprocessor::run(std::vector<Line*>& lines) {
	debug("=== Stage 2: Preprocessor.run() ===");
	
	//	Iterate through each Line in the lines vector generated by the Parser.
	for (Line* line : lines) {
		switch (line->type) {
...
void Preprocessor::handleDirective(DirectiveLine* line) {
	...
	switch (line->directive->type) {
		...
		case DirectiveType::start:
			this->handleStartDirective(line);
			break;
		default:
...
void Preprocessor::handleDefineDirective(DirectiveLine* line) {
	...
	converter << std::hex << constant.value;
	std::string constantValueString = converter.str();
	debug("DEFINED CONSTANT " + constant.name + " WITH VALUE " + constantValueString + "!");

	//	Add constant to constants hashmap
	this->constants[constant.name] = constant;
}

void Preprocessor::handleStartDirective(DirectiveLine* line) {
	debug("handleStartDirective()");

	//	Get StartDirective arguments.
	//	Tokens: .start START_ADDRESS

	long long addressValue = getNumberValue(line->tokens[1].token_string);

	if (addressValue < 0 || addressValue > MAX_ADDRESS) {
		//	Start address is out of range.
		throwErrorLine(line->number,
			"Start address is out of valid range (0x0000 - 0xDFFF).");
	}

	//	Set start address in config
	config.start_address = static_cast<uint16_t>(addressValue);
}