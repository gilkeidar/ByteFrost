...
#ifdef _WIN32
/* When the project is linked as a GUI application the linker looks for WinMain.
   Provide a small wrapper that forwards to the standard main using MSVC's
   __argc/__argv globals. */
extern int main(int argc, char **argv);
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    (void)hInstance; (void)hPrevInstance; (void)lpCmdLine; (void)nCmdShow;

    // Try to attach to parent console; if that fails, allocate a new console.
...
void pack() {
    ...
    //  Populate boot block range (blocks 0-7)
...
DiskDirectory * getDiskDirectory(char * directory_name) 
{
    ...
    char search_path[MAX_FILE_NAME_LEN];
    WIN32_FIND_DATAA fd;
    HANDLE hFind;
    ...
    do {
        printf("\t%s\n", fd.cFileName);
        if (strcmp(fd.cFileName, FILES_DIR_NAME) == 0) {
            printf("Files directory found!\n");
            files_dir_found = TRUE;
        } else if (strcmp(fd.cFileName, BOOT_FILE_NAME) == 0) {
            printf("Boot file found!\n");
            boot_file_found = TRUE;
        }
    ...
    HANDLE hFindFiles;
    char files_search[MAX_FILE_NAME_LEN];
    snprintf(files_search, sizeof(files_search), "%s\\*", files_path);
    hFindFiles = FindFirstFileA(files_search, &ffd);
    ...
    do {
        ...
        if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            // skip directories inside files/ (shouldn't be any files here)
            continue;
        }

        printf("\t%s\n", ffd.cFileName);

        char ** file_name_split = splitFileExtension(ffd.cFileName);
        ...
        strcpy(current_file_path, files_path);
        strcat(current_file_path, "\\");
        strcat(current_file_path, ffd.cFileName);

        printf("Current file's path: %s\n", current_file_path);

        if ((current_file = fopen(current_file_path, "rb")) == NULL) {
            fprintf(stderr, "Error: Couldn't open file '%s'.\n", 
                current_file_path);
            exit(FILE_FAIL);
        }
        ...
        disk_directory->file_sizes[disk_directory->num_files] = current_file_size;
        disk_directory->num_files++;
    } while (FindNextFileA(hFindFiles, &ffd));
    ...
    if ((boot_file = fopen(boot_file_path, "rb")) == NULL) {
        fprintf(stderr, "Error: Couldn't open file '%s'.\n", 
            boot_file_path);
        exit(FILE_FAIL);
    }
...
FILE * createBFSDiskv1(char * disk_name) {
    ...
    for (int i = 0; i < disk_size; i++) {
        if ((bytesWritten = (int) fwrite(buf, sizeof(char), sizeof(buf)/sizeof(buf[0]), disk)) !=  1024) {
...
void writeBootFile(DiskDirectory * disk_dir, FILE * disk) {
    ...
    int  bytesRead;
    size_t totalRead = 0;              //  Number of bytes read in each iteration
    ...
    // determine boot file size
    if (fseek(disk_dir->boot_file, 0, SEEK_END) != 0) {
        fprintf(stderr, "Error: Failed to seek boot file.\n");
        exit(UNEXPECTED_ERROR);
    }
    long boot_file_size = ftell(disk_dir->boot_file);
    if (boot_file_size < 0) {
        fprintf(stderr, "Error: Failed to tell boot file size.\n");
        exit(UNEXPECTED_ERROR);
    }
    rewind(disk_dir->boot_file);

    while ((bytesRead = (int) fread(buf, 1, BLOCK_SIZE, disk_dir->boot_file)) > 0) {
        //  Write boot file contents to the disk
        printf("bytesRead: %d\n", bytesRead);
        if (fwrite(buf, 1, bytesRead, disk) != bytesRead) {
            fprintf(stderr, "Error: Failed to write boot data to disk.\n");
            exit(UNEXPECTED_ERROR);
        }
        totalRead += bytesRead;
    }
    if ((size_t)boot_file_size != totalRead)
    {
        fprintf(stderr, "Error: fread(boot) %zu does not match boot file size %ld.\n", totalRead, boot_file_size);
        exit(UNEXPECTED_ERROR);
    }
}
...
void writeFileToDisk(DiskDirectory * disk_dir, int fileIndex, FILE * disk) {
    ...
    while ((bytesRead = (int) fread(buf, 1, BLOCK_SIZE, file)) > 0) {
...
int allocateInode(FILE * disk) {
    ...
    unsigned char cur_free_inodes[1];   //  Current byte of the freemap
    int free_inode = 0;
    int bytesRead;

    for (int i = 0; i < INODE_FREE_MAP_LEN; i++) {
        if ((bytesRead = (int) fread(cur_free_inodes, sizeof(char), 1/sizeof(char), disk)) != 1) {
        ...
        while (mask != 0) {
            if ((mask & cur_free_inodes[0]) == 0) {
                ...
                //  Mark that inode is used up in the inode freemap
                ...
                return free_inode + FIRST_INODE_BLOCK;  // inode blcok can be in the range of 11 .. 26
            }
            free_inode++;
...
int allocateDataBlock(FILE * disk) {
    ...
    unsigned char cur_free_map[1];   //  Current byte of the freemap
    int free_data_block = 0;
    int bytesRead;

    for (int i = 0; i < DATA_FREE_MAP_LEN; i++) {
        if ((bytesRead = (int) fread(cur_free_map, 1, 1, disk)) != 1) {
            fprintf(stderr, "Error: fread() failed; bytesRead = %d. error: %s\n",
                bytesRead, strerror(errno));
            exit(UNEXPECTED_ERROR);
        }
        ...
        while (mask != 0) {
            if ((mask & cur_free_map[0]) == 0) {
                ...
                printf("Found free block # %d\n", free_data_block + FIRST_DATA_BLOCK);
...
char ** splitFileExtension(char * filename) {
    ...
    int name_length = (int) strlen(filename_copy);
...
void allocateDirectoryEntry(char * filename, int inode_block, FILE * disk) {
    ...
    if ((bytesRead = (int) fread(directory_block, 1, BLOCK_SIZE, disk)) != BLOCK_SIZE) {
    ...
    for (int i = 0; i < MAX_NUM_FILES; i++) {
        ...
        if (current_entry[14] == 0 && current_entry[15] == 0) {
            ...
            if ((bytesWritten = (int) fwrite(current_entry, 1, DIRECTORY_ENTRY_SIZE, disk)) != DIRECTORY_ENTRY_SIZE) {
...
void setInodeMetadata(FileType type, int inode_block, FILE * disk) {
    ...
    char buf[1] = {type};
    int bytesWritten;
    if ((bytesWritten = (int) fwrite(buf, 1, 1, disk)) != 1) {
